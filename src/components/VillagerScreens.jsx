...
export function HomeScreen({ setActive }) {
  ...
  return (
    <div className="animate-fadeInUp">
      ...
      {/* ── Suitable Crops – RIGHT after banner, visible on mobile without scrolling ── */}
      {roleMode === 'farmer' && districtCropsMap[normalizeDistrict(userDistrict)] && (
        <div className="animate-fadeInUp card" style={{ marginBottom: 20, padding: 0 }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border-light)', display: 'flex', alignItems: 'center', gap: 8 }}>
            <Sprout size={17} style={{ color: 'var(--primary)', flexShrink: 0 }} />
            <div>
              <h4 style={{ fontSize: 14, fontWeight: 700, margin: 0, color: 'var(--text-primary)' }}>
                {lang === 'kn' ? `${userDistrict} ಜಿಲ್ಲೆಗೆ ಸೂಕ್ತವಾದ ಬೆಳೆಗಳು` : `Suitable Crops — ${userDistrict}`}
              </h4>
              <p style={{ fontSize: 11, color: 'var(--text-muted)', margin: '2px 0 0 0' }}>
                {lang === 'kn' ? 'ತೊಟ್ಟಿಕೆ ಮಾಡಿ ವಿವರ ನೋಡಿ' : 'Tap a crop for details'}
              </p>
            </div>
          </div>
      ...
      {/* ── Stats Grid (Active Schemes, My Complaints, New Alerts) ── */}
      ...
      {roleMode === 'student' && (
        <div className="animate-fadeInUp card" style={{ marginBottom: 20, border: '1.5px solid var(--primary-light)', background: 'var(--bg-card)' }}>
          ...
          {/* Dynamic deadline status helper — auto-updates based on today's date */}
          {(() => {
            const today = new Date();
            today.setHours(0, 0, 0, 0);

            const getDeadlineStatus = (deadlineStr, openDateStr) => {
              if (!deadlineStr) return { label: 'Check Website', color: '#64748b', bg: '#f1f5f9', icon: '🔗' };
              ...
              if (openDateStr) {
                ...
                if (today < openDate) return { label: `Opens ${openDate.toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}`, color: '#7c3aed', bg: '#f5f3ff', icon: '🔔' };
              }
              if (daysLeft < 0) return { label: 'Deadline Passed — Reference Only', color: '#dc2626', bg: '#fee2e2', icon: '❌' };
              if (daysLeft <= 15) return { label: `Closing Soon — ${daysLeft}d left`, color: '#d97706', bg: '#fef9c3', icon: '⚠️' };
                  ...
                  return (
                    <div key={sIdx} style={{ background: 'var(--bg-main)', borderRadius: 12, padding: 16, border: `1px solid ${isPassed ? '#fca5a5' : 'var(--border-light)'}`, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', boxShadow: '0 2px 6px rgba(0,0,0,0.04)', opacity: isPassed ? 0.82 : 1 }}>
                      <div>
                        ...
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: status.bg, color: status.color, fontSize: 10, fontWeight: 800, padding: '3px 8px', borderRadius: 20, marginBottom: 8, border: `1px solid ${status.color}30` }}>
                      ...
                      <a
                        ...
                        className="btn btn-primary btn-sm"
                        style={{ width: '100%', justifyContent: 'center', fontSize: 12, padding: '8px 12px', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 6, fontWeight: 600, borderRadius: 8, background: isPassed ? '#64748b' : undefined }}
                      >
                        {isPassed ? (lang === 'kn' ? 'ವಿವರ ನೋಡಿ (ಮೊದಲ ದಿನಾಂಕ ಮೀರಿದೆ)' : 'View Details (Deadline Passed)') : (lang === 'kn' ? 'ಈಗ ಅರ್ಜಿ ಸಲ್ಲಿಸಿ →' : 'Apply Now →')}
                      </a>
                    </div>
                  );
                })}
            ...
            {filteredSchemes.map((s, idx) => (
              ...
              >
                ...
                <div style={{ padding: '16px 18px', display: 'flex', flexDirection: 'column', flex: 1 }}>
                  ...
                  {/* Inline Scheme Details (Opens nearby area inside the card) */}
                  {selectedScheme && selectedScheme.id === s.id && (
                    <div style={{
                      marginTop: 16,
                      paddingTop: 16,
                      ...
                      <button
                        ...
                        onClick={() => handleToggleVoice(s)}
                        style={{ display: 'inline-flex', alignItems: 'center', gap: 6, borderRadius: 8, fontSize: 12, width: 'fit-content' }}
                      >
                        <Volume2 size={14} />
                      ...
                      {/* Full description */}
                      <div>
                        <h5 style={{ fontWeight: 700, fontSize: 13, marginBottom: 4, color: 'var(--text-primary)' }}>Description</h5>
                      ...
                      {s.benefits && (
                        <div style={{
                          ...
                          <h5 style={{ color: '#065f46', fontWeight: 800, fontSize: 13, margin: '0 0 6px 0', display: 'flex', alignItems: 'center', gap: 4 }}>
                            <IndianRupee size={14} /> Benefit & Subsidy Structure
                          </h5>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                            <div><strong>Subsidy:</strong> {s.benefits.subsidyPercent || s.benefits.maxLimit}</div>
                            {s.benefits.mode && <div><strong>Mode:</strong> {s.benefits.mode}</div>}
                            {s.benefits.interestSubvention && s.benefits.interestSubvention !== 'N/A' && (
                              <div><strong>Interest Subvention:</strong> {s.benefits.interestSubvention}</div>
                            )}
                          </div>
                        </div>
                      )}
                      ...
                      {/* Eligibility */}
                      <div style={{ background: 'var(--bg-main)', padding: 12, borderRadius: 10, border: '1px solid var(--border-light)', fontSize: 12 }}>
                        <h5 style={{ fontWeight: 700, fontSize: 13, color: 'var(--primary)', margin: '0 0 4px 0' }}>
                          Eligibility Criteria
                        </h5>
                        <p style={{ margin: 0, color: 'var(--text-primary)', lineHeight: 1.4 }}>
                          {getLangText(s.eligibility)}
                        </p>
                      </div>

                      {/* Exclusions */}
                      {s.exclusions && (
                        <div style={{ background: '#fef2f2', padding: 12, borderRadius: 10, border: '1px solid #fecaca', fontSize: 12 }}>
                          <h5 style={{ fontWeight: 700, fontSize: 12, color: '#991b1b', margin: '0 0 4px 0' }}>
                            Exclusions
                          </h5>
                          <p style={{ margin: 0, color: '#7f1d1d', lineHeight: 1.4 }}>
                            {getLangText(s.exclusions)}
                          </p>
                        </div>
                      )}
                      ...
                      {/* Documents */}
                      <div>
                        <h5 style={{ fontWeight: 700, fontSize: 13, marginBottom: 4, color: 'var(--text-primary)' }}>Required Documents</h5>
                        <p style={{ fontSize: 12, color: 'var(--text-secondary)', margin: 0, lineHeight: 1.4 }}>
                          {getLangText(s.documents)}
                        </p>
                      </div>
                      ...
                      {s.processSteps && (
                        <div>
                          <h5 style={{ fontWeight: 700, fontSize: 13, marginBottom: 8, color: 'var(--text-primary)' }}>Application Process</h5>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                            {s.processSteps.map(st => (
                              <div key={st.step} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                                <span style={{
                                  width: 20,
                                  height: 20,
                                  borderRadius: '50%',
                      ...
                      {s.timeline && (
                        <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                          Processing Timeline: <strong>{s.timeline}</strong>
                        </div>
                      )}
                      ...
                      {s.trackerUrl && (
                        <a
                          ...
                          style={{ textDecoration: 'none', justifyContent: 'center', fontSize: 11, width: 'fit-content', marginTop: 6 }}
                        >
                          Track Portal Status ↗
                        </a>
                      )}
